...
$(() => {
  ...
  const addOrderItem = (object) => {
    const $orderItem = $(`
      <tr id="${object.name}">
        <td>${object.name}</td>
        <td><button class="minus">-</button><input class='quantity' type="text" value="${
          object.quantity
        }" min="1" max="9" /><button class="add">+</button></td>
        <td>$${object.price}</td>
  ...
  const renderRow = (rowNum) => {
    const activeStatus =
      rowNum === 1 ? "carousel-item active" : "carousel-item";
    return $(`
  ...
  const loadItems = () => {
    $.ajax({
      url: "https://spuddies-food-ordering.herokuapp.com/api/menu/",
      method: "GET",
      dataType: "json",
    })
      .then((data) => {
